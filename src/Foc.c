...
void foc_int(void)
{
	...
	angle_step = 1;//11 degree/ms 
	angle_current = 0;
	
	motor_1.angle_cos = 0;
	motor_1.angle_sin = 0;
	motor_1.pwm_period = MAIN_ISR_PERIOD;
...
void theta_calculate(t_motor * pm)
{
	...
	if(angle_current >= 360.0)
		angle_current = 0;

	pm->angle_rad = (angle_current) * (float)(2.0 * M_PI / 360.0);
	pm->angle_sin = sinf(pm->angle_rad);
	pm->angle_cos = cosf(pm->angle_rad);
}
...
//	pm->out_w = (float) (0.2);
	...
	//	  if (pm->charge_state == BS_CHARGE_END)
	{
		...
		/* Update PWM timers */
		pm->PhaseU_Reg = (unsigned short) (pm->pwm_period_half - (short) _u);
		pm->PhaseV_Reg = (unsigned short) (pm->pwm_period_half - (short) _v);
		pm->PhaseW_Reg = (unsigned short) (pm->pwm_period_half - (short) _w);

		PWM_CMP_UPDATE_MACRO(pm->PhaseU_Reg, pm->PhaseV_Reg, pm->PhaseW_Reg);
